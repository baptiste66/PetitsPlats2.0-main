...
const valeur = "concombre";
    ...
    while (i < recipes.length) {
        const { name, ingredients, description, image, time } = recipes[i];
    ...
    if (valeur.length > 3) {
        const filteredRecipes = recipes.filter((recipe) => {
            ...
            return name.toLowerCase().includes(valeur) || description.toLowerCase().includes(valeur) || ingredients.some(ingredient => ingredient.ingredient.toLowerCase().includes(valeur));
        });
    }